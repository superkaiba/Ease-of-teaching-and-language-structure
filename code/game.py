...
class GuessGame:
    def __init__(self, args):
        ...
        self.rbot = models.MLPReceiver(args).to(self.device)
    ...
    def forward(self, targets, targets_idx, candidates, evaluate=False, sOpt=True, rOpt=True, stochastic=True): # sOpt equals false only for non-nn models
        ...
        return loss, accuracy, m, predicted # evaluate_probs, pred_probs for probing languages

    def backward(self, loss, sOpt=True, rOpt=True): # sOpt is false when we do not need update sender
        if sOpt:
